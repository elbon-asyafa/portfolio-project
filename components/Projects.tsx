...
const P=[
  ...
  {id:2,tag:"Winbox & Troubleshooting",title:"Connectivity Fault Resolution",desc:"Structured troubleshooting using Winbox, ping testing, and DHCP checks to identify and resolve live connectivity issues affecting multiple endpoints.",detail:"Layered OSI approach: physical link → DHCP lease → routing table → application layer. Fast root cause isolation to prevent recurrence.",tech:["Winbox","Ping & Tracert","DHCP Debug","MikroTik","OSI Model"],imgs:[{src:"/images/projects/troubleshooting/1.webp",l:"Winbox UI",icon:"🖥️",bg:"rgba(74,100,144,0.15)"},{src:"/images/projects/troubleshooting/2.webp",l:"Ping Test",icon:"⚡",bg:"rgba(74,100,144,0.10)"},{src:"/images/projects/troubleshooting/3.webp",l:"Fault Check",icon:"🔍",bg:"rgba(110,136,176,0.12)"}]},
  {id:3,tag:"Web Development",title:"Ruz Stationary — E-Commerce",desc:"Built a full online stationery store with a friend (Haeru Rizky) using plain HTML, CSS, and JavaScript. Hosted on InfinityFree.",detail:"Features: live review system via JSONBin, email notifications via Formspree, wishlist drawer, product photos with emoji fallback, loading screen, social media integration.",tech:["HTML","CSS","JavaScript","JSONBin","Formspree","InfinityFree"],imgs:[{src:"/images/projects/web/1.webp",l:"Homepage",icon:"🛒",bg:"rgba(242,224,208,0.35)"},{src:"/images/projects/web/2.webp",l:"Products",icon:"🖊️",bg:"rgba(242,224,208,0.25)"},{src:"/images/projects/web/3.webp",l:"Reviews",icon:"⭐",bg:"rgba(212,184,152,0.25)"}]},
];

function ImgGrid({imgs,title}:{imgs:typeof P[0]["imgs"];title:string}) {
  return(
    <div className="grid grid-cols-2 gap-2.5">
      <div className="col-span-2 img-zoom relative h-48 sm:h-56 rounded-2xl overflow-hidden"
        ...
        <Image src={imgs[0].src} alt={`${title}-${imgs[0].l}`} fill className="object-cover z-10" onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>
      ...
      {imgs.slice(1).map((img,i)=>(
        <div key={i} className="img-zoom relative h-28 sm:h-32 rounded-xl overflow-hidden"
          ...
          <Image src={img.src} alt={`${title}-${img.l}`} fill className="object-cover z-10" onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>
