  ...
  return (
    <div className="space-y-3">
      ...
      {[
        ...
        { l:"Location",  v:"Pasarkemis, West Java" },
        { l:"Instagram", v:"@asyworst" },
        { l:"TikTok",    v:"@asyaffarn" },
      ].map(r => (
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {cat.slots.map((s, i) => (
              <div key={i} className="relative aspect-square rounded-xl overflow-hidden group img-zoom"
                ...
                <Image src={s.src} alt={s.l} fill className="object-cover z-10"
...
function ContactContent() {
  ...
  const links = [
    ...
    { icon:"🎵", label:"TikTok",    val:"@asyaffarn",       href:"https://tiktok.com/@asyaffarn" },
    { icon:"✉️", label:"Email",     val:"elbon@example.com",href:"mailto:elbon@example.com" },
  ];
  return (
    <div className="space-y-1">
      ...
      <button
        onClick={() => { navigator.clipboard.writeText("elbon@example.com"); setCp(true); setTimeout(() => setCp(false), 2000); }}
...
function WinPanel({ win, onClose, onFocus }: { win:Win; onClose:(id:string)=>void; onFocus:(id:string)=>void }) {
  ...
  const glass: React.CSSProperties = {
    ...
    backdropFilter:     "blur(28px) saturate(2.0) brightness(1.04)",
    WebkitBackdropFilter:"blur(28px) saturate(2.0) brightness(1.04)",
    borderTop:    "1px solid rgba(255,255,255,0.90)",
    borderLeft:   "1px solid rgba(255,255,255,0.55)",
    borderRight:  "1px solid rgba(255,255,255,0.30)",
    borderBottom: isTouch ? "none" : "1px solid rgba(255,255,255,0.15)",
    boxShadow:    "0 16px 64px rgba(74,100,144,0.22), 0 4px 16px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.92)",
  };
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <button onClick={onClick}
        ...
        style={{
          background:   active ? "rgba(74,100,144,0.30)" : "rgba(255,255,255,0.22)",
          ...
          boxShadow: active
            ? "0 0 0 2px rgba(74,100,144,0.40), 0 6px 20px rgba(74,100,144,0.20), inset 0 1px 0 rgba(255,255,255,0.88)"
            : "0 4px 14px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.88)",
        }}>
        {icon}
      </button>
...
/* ── Main ── */
export default function WindowPopups() {
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes dockUp  { from{opacity:0;transform:translateX(-50%) translateY(28px)} to{opacity:1;transform:translateX(-50%) translateY(0)} }
      `}</style>
      ...
      {/* ── Liquid Glass Dock ── */}
      <div className="fixed bottom-20 left-1/2 z-[500]"
        style={{
        ...
        {/* Outer glow */}
        <div className="absolute -inset-3 rounded-[32px] blur-xl opacity-35 pointer-events-none"
          style={{ background:"linear-gradient(135deg,rgba(242,224,208,0.60),rgba(74,100,144,0.40))" }} />

        <div className="relative flex items-end gap-2 px-4 py-3 rounded-[26px]"
          style={{
            background:         "rgba(255,255,255,0.26)",
            backdropFilter:     "blur(28px) saturate(2.0) brightness(1.06)",
            WebkitBackdropFilter:"blur(28px) saturate(2.0) brightness(1.06)",
            ...
            boxShadow:    "0 12px 48px rgba(74,100,144,0.22), 0 3px 12px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.95)",
          }}>
          {wins.map(w => (
